...
- `qmethod()`: Specifies the method used to estimate the quantile regression component. The default is `qreg`, but any other quantile regression command can be used. This will affect how `quantile(#)` is specified.
...
-  `abs(varlist)`: specifies the variables that would be used to absorb the fixed effects. This is necessary for CRE
