...
Where $U_{it}$ is a random variable that follows a uniform distribution, and $\alpha_i(U_{it})$ is the unobserved effect that varies across individuals. In this case, the conditional quantile regression model can be written as: $$q_\tau(y_{it}|x_{it},\alpha_i(\tau))=\beta_0(\tau) + x_{it}'\beta(\tau)+\alpha_i(\tau) 
$$ {#eq-feqr}
...
Although of little use, this location-scale model can be easily estimated using a two-step approach. First, estimate the location effect of $x_i$ on $y_i$ using OLS: $$y_i = \beta_0+x_i'\beta + \mu_i
$$ {#eq-ls-1}
...
1.  Estimate the location effect of $x_{it}$ and the unobserved heterogeneity $\alpha_i$ on $y_{it}$ using OLS: $$y_{it}=\beta_0 +  x_{it}' \beta_1   +\alpha_i + \varepsilon_{it}
    $$
