...
As explained in @wooldridge2010, the coefficients of quantile regression models can be identified by minimizing the following loss function, with respect to $\beta(\tau)$:
...
$$q_\tau(y_{it}|x_it,\alpha_i) = \beta_0(\tau) + x_{it}\beta(\tau) + \alpha_i$${#eq-canay-qreg}
...
$$y_{it} = \beta_0(U_{it}) + x_{it}'\beta(U_{it}) + \alpha_i + \alpha_t$${#eq-canay-dgp-2way}
...
$$q_{\tau}(y_{it}-\hat \alpha_i - \hat \alpha_t|x_{it})=q_{\tau}(\tilde y_{it}|x_{it}) = x_{it}\beta(\tau)$${#eq-canay-mway}
...
$$y_{it} =\beta_0 + x_{it}'\beta + \beta_\alpha \alpha_i+ \gamma_0(U_{it}) + x_{it}'\gamma(U_{it}) + \gamma_\alpha(U_{it}) \alpha_i$${#eq-mcanay-dgp}
